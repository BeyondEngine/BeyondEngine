...
#if $current_class.has_public_destructor
...
int lua_register_${generator.prefix}_${current_class.class_name}(lua_State* tolua_S)
{
    ...
    #for m in st_methods
        ...
        tolua_variable(tolua_S, "$f.name", lua_${current_class.namespaced_class_name}_${f.name}_getter, lua_${current_class.namespaced_class_name}_${f.name}_setter);
        #end for
    #end if
