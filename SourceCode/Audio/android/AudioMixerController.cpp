    ...
    for (auto&& track : _activeTracks)
    {
        ...
        if (state == Track::State::PLAYING)
        {
            ...
            if (track->isVolumeDirty())
            {
                ...
                BEATS_PRINT("Track (name: %d)'s volume is dirty, update volume to L: %f, R: %f", name, lVolume, rVolume);
#endif
        ...
        else if (state == Track::State::RESUMED)
        {
            ...
            {
                BEATS_PRINT("Previous state (%d) isn't PAUSED, couldn't resume!", static_cast<int>(track->getPrevState()));
            }
#endif
        }
        else if (state == Track::State::PAUSED)
        {
            ...
            {
                BEATS_PRINT("Previous state (%d) isn't PLAYING, couldn't pause!", static_cast<int>(track->getPrevState()));
            }
#endif
        }
        else if (state == Track::State::STOPPED)
        {
            ...
            {
                BEATS_PRINT("Track (%p) hasn't been initialized yet!", track);
            }
#endif
            tracksToRemove.push_back(track);
        }
