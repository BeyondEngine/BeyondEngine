...
static int getSystemAPILevel()
{
    ...
    int apiLevel = CAndroidHandler::GetInstance()->GetSDKVersion();
    if (apiLevel > 0)
    {
...
void AudioPlayerProvider::preloadEffect(const std::string &audioFilePath, const PreloadCallback& cb)
{
    ...
    preloadEffect(info, [this, cb](bool succeed, PcmData data){
        cb(succeed, data);
    }, false);
}
        ...
        const std::string &audioFilePath)
{
    ...
    strPath.append(_T("/")).append(audioFilePath);
    if (strPath[0] != '/')
    {
        std::string relativePath;
        
        size_t position = strPath.find("assets/");
        if (!CFilePathTool::GetInstance()->Exists(strPath.c_str()))
        {
            BEATS_PRINT("audio file %s is not found!", strPath.c_str());
        }
        if (0 == position)
        {
        ...
        BEATS_PRINT("get file descriptor for '%s'", relativePath.c_str());
        assetFd = _fdGetterCallback(relativePath, &start, &length);

        if (assetFd <= 0)
        {
            BEATS_PRINT("Failed to open file descriptor for '%s'", strPath.c_str());
            return info;
        }
                                                           ...
                                                           const PcmData &pcmData)
{
    ...
    if (pcmData.isValid())
    {
        pcmPlayer = new(std::nothrow) PcmAudioPlayer(_mixController);
