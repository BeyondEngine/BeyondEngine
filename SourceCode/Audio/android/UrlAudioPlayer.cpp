...
std::vector<UrlAudioPlayer*> __playerContainer;
...
UrlAudioPlayer::UrlAudioPlayer(SLEngineItf engineItf, SLObjectItf outputMixObject)
        : _engineItf(engineItf), _outputMixObj(outputMixObject), _id(-1), _assetFd(nullptr),
          ...
          _playEventCallback(nullptr), _isDestroyed(false)
{
...
void UrlAudioPlayer::playEventCallback(SLPlayItf caller, SLuint32 playEvent)
{
    ...
    if (playEvent == SL_PLAYEVENT_HEADATEND)
    {
        ...
        else
        {
            ...
            ALOGV("UrlAudioPlayer (%p) played over, will destroy self...", this);
        }
    }
}
...
bool UrlAudioPlayer::delayDelete()
{
    return true;
}
...
void UrlAudioPlayer::destroy()
{
    ...
    {
        _isDestroyed = true;
        BEATS_PRINT("UrlAudioPlayer::destroy() %d", getId());
        SL_DESTROY_OBJ(_playObj)
        BEATS_PRINT("UrlAudioPlayer::destroy end");
    }
}
