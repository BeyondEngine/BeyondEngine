...
class NativeField(object):
    def __init__(self, cursor):
        ...
        self.type = NativeType.from_type(self.cursor.type);
...
class NativeFunction(object):
    def __init__(self, cursor):
        self.cursor = cursor
        self.is_struct_default_constructor = cursor.kind == cindex.CursorKind.STRUCT_DECL #it means this is a default struct constructor
        self.func_name = cursor.displayname if self.is_struct_default_constructor else cursor.spelling
        self.signature_name = self.func_name if self.is_struct_default_constructor else self.func_name
        self.arguments = []
        self.argumtntTips = []
        self.static = False if self.is_struct_default_constructor else cursor.kind == cindex.CursorKind.CXX_METHOD and cursor.is_static_method()
        self.implementations = []
        self.is_constructor = True if self.is_struct_default_constructor else False
        self.not_supported = False
        self.is_override = False
        self.ret_type = NativeType.from_string("void") if self.is_struct_default_constructor else NativeType.from_type(cursor.result_type)
        self.comment = None if self.is_struct_default_constructor else self.get_comment(cursor.getRawComment())
        self.min_args = 0
        if not self.is_struct_default_constructor:
            # parse the arguments
...
class NativeClass(object):
    def __init__(self, cursor, generator):
        ...
        self.cursor = cursor
        self.is_struct = self.cursor.kind == cindex.CursorKind.STRUCT_DECL
        ...
        self.has_public_destructor = True
        self.namespace_name = ""
    ...
    def generate_code(self):
        ...
        if not self.has_constructor and self.cursor.kind == cindex.CursorKind.STRUCT_DECL:
            defaultConstructor = NativeFunction(self.cursor)
            self.has_constructor = True
            if not self.methods.has_key('constructor'):
                self.methods['constructor'] = defaultConstructor
        ...
        if self.generator.script_type == "lua":
            if self.is_struct:
                structAccess = Template(file=os.path.join(self.generator.target, "templates", "varsetterandgetter.c"),
                                searchList=[self])
                self.generator.impl_file.write(str(structAccess))
        ...
        if cursor.kind == cindex.CursorKind.CXX_BASE_SPECIFIER:
            ...
            if not self.methods.has_key('constructor'):
                ...
                if isinstance(previous_m, NativeOverloadedFunction):
                    ...
                    self.methods['constructor'] = m
            return True
        elif self._current_visibility != cindex.AccessSpecifierKind.PUBLIC and cursor.kind == cindex.CursorKind.DESTRUCTOR:
            self.has_public_destructor = False
            return True
        # else:
...
class Generator(object):
    def __init__(self, opts):
        ...
        self.headerpath = opts['headerpath']
    ...
    def _parse_headers(self):
        for header in self.headers:
            tu = self.index.parse(self.headerpath + header, self.clang_args)
    ...
    for t in targets:
        ...
        for s in sections:
            ...
            gen_opts = {
                ...
                'headerpath': (config.get(s, 'headerpath', 0, dict(userconfig.items('DEFAULT')))),
